...
static inline void
String_setup_skip_charrs(size_t* skip_chars, const unsigned char* needle,
                         ssize_t nlen)
{
    size_t i = 0;
    size_t last = nlen - 1;
...
int String_find(bstring in, bstring what)
{
    ...
    String_setup_skip_charrs(skip_chars, needle, nlen);
...
static inline void
StringScanner_set_needle(StringScanner* scan, bstring tofind)
{
    scan->needle = (const unsigned char*)bdata(tofind);
    scan->nlen = blength(tofind);

    String_setup_skip_charrs(scan->skip_chars, scan->needle, scan->nlen);
}
