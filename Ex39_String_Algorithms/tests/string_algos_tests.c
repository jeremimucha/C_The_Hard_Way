...
char* test_find_and_scan()
{
    ...
    scan_i = StringScanner_scan(scan, &ALPHA);
    fprintf(stderr, "scan_i = %d\n", scan_i);
    mu_assert(scan_i > find_i, "should find another 'ALPHA' after the first 2");
...
char* test_find_performance()
{
    ...
    debug("FIND COUNT: %lu, END_TIME: %d, OPS: %f",
          find_count, (int)elapsed, (double)find_count / elapsed);
...
char* test_scan_performance()
{
    ...
    debug("SCAN COUNT: %lu, END_TIME: %d, OPS: %f",
          find_count, (int)elapsed, (double)find_count/elapsed);
