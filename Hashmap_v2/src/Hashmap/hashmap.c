...
/**
 * Simple Bob Jenkins' hash algorithim from wiki
 */
...
static inline HashmapNode* Hashmap_node_create(int hash, Key key, Value data)
{
...
static inline DArray* Hashmap_find_bucket(Hashmap* map, Key key,
                                        int create, uint32_t* hash_out)
{
uint32_t hash = map->hash(key);
int bucket_n = hash % DEFAULT_NUM_BUCKETS;
...
if(!bucket && create){
    // set up new bucket
    bucket = DArray_create(sizeof(void*), DEFAULT_NUM_BUCKETS);
    check_mem(bucket);
